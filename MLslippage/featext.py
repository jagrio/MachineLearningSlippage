...
def sentr(f):
    ...
    counts = [np.bincount(np.abs(np.int_(res*(f[:,i]-np.mean(f[:,i]))/np.std(f[:,i])))) for i in range(f.shape[1])]
...
def arco(f):
    if len(f.shape)<=1:
        ...
        arco = np.array([alg.AR_est_YW(f[:,i],p)[0] if sum(abs(f[:,i]))>1e-5 else np.zeros(f[:,i].shape) for i in range(f.shape[-1])])
...
def feat(f,havelabel=0,featlabel=0,magnFFT=0,featall=0):
    ...
    # |------------|------> initial : 11+3{shist} -----------------------------> = 14+0.0samples ||             14 #
    # |------------|------> golz's  : 10+samples{acorl} -----------------------> = 10+1.0samples ||           1034 #
    # |----------> frequency domain :                                                                              #
    # |------------|------> initial : 3{arco}+4{mf}+2(samples/2+1){RF,IF} -----> =  9+1.0samples ||           1033 #
    # |------------|------> golz's  : 1{ffaf}+2(samples/2+1){AF,PF} -----------> =  3+1.0samples ||           1027 #
